...
public class Personne {
    ...
    public void setEmail(String eMail) throws IllegalArgumentException
    {
        ...
        ArrayList<String> emailParts = new ArrayList<>(Arrays.asList(eMail.split("\\.")));
        String emailTLD = emailParts.get(emailParts.size()-1).toUpperCase();
        String emailRest = eMail.substring(0, eMail.length()-emailTLD.length()-1);

        if((!TldList.contains(emailTLD)) || "".equals(emailRest) || eMail.chars().filter(ch -> ch == '@').count()!=1 || eMail.matches(".*[:;=+\"\\_].*")){
